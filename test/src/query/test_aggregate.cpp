...
static auto make_db() {
    return atlas::make_storage(atlas::make_table<User>("users",
...
ut::suite<"query/aggregate"> aggregate_suite = [] {
    ...
    "count() works on string column"_test = [] {
        ...
        static_assert(std::is_same_v<decltype(agg), atlas::count_expr<atlas::column_ref<User, std::string>>>);
        expect(true);
    };
    ...
    "sum() produces sum_expr"_test = [] {
        ...
        static_assert(std::is_same_v<decltype(agg), atlas::sum_expr<atlas::column_ref<User, double>>>);
        expect(agg.col.ptr == &User::score);
    };

    "avg() produces avg_expr"_test = [] {
        auto agg = atlas::avg(&User::score);
        static_assert(std::is_same_v<decltype(agg), atlas::avg_expr<atlas::column_ref<User, double>>>);
        expect(true);
    };

    "min() produces min_expr"_test = [] {
        auto agg = atlas::min(&User::age);
        static_assert(std::is_same_v<decltype(agg), atlas::min_expr<atlas::column_ref<User, int32_t>>>);
        expect(true);
    };

    "max() produces max_expr"_test = [] {
        auto agg = atlas::max(&User::age);
        static_assert(std::is_same_v<decltype(agg), atlas::max_expr<atlas::column_ref<User, int32_t>>>);
        expect(true);
    };
    ...
    "select(count()) accepts COUNT(*) expression"_test = [] {
        auto q = atlas::select(atlas::count()).from<User>().where(atlas::gt(&User::age, 18));

        static_assert(atlas::is_aggregate<std::tuple_element_t<0, decltype(q.selected)>>);
        expect(true);
    };

    "select(count(col)) accepts COUNT(col) expression"_test = [] {
        auto q = atlas::select(atlas::count(&User::id)).from<User>();
